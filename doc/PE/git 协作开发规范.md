...
> **CI**  
CI 是 Continuous Integration 的缩写，表示在代码提交或 PR 创建后，由 GitHub Actions 自动执行编译、测试等流程。CI 通过后，代码才具备进入主分支的基本条件。
...
PR 创建后，GitHub Actions 会自动触发 CI 检查。项目中的 CI 会执行编译和单元测试。

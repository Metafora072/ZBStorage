...
IO_LATENCY_ENABLED="${IO_LATENCY_ENABLED:-false}"
IO_LATENCY_DIR="${IO_LATENCY_DIR:-}"
...
# The generated file is sourced by bash; quote paths (including spaces) as data.
for io_latency_key in IO_LATENCY_ENABLED IO_LATENCY_DIR IO_LATENCY_FLUSH_MS IO_LATENCY_BATCH_RECORDS IO_LATENCY_QUEUE_CAPACITY; do
  printf -v "${io_latency_key}_SHELL" '%q' "${!io_latency_key}"
  export "${io_latency_key}_SHELL"
done
